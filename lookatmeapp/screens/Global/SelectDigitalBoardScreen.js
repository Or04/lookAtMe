...
class SelectDigitalBoardScreen extends React.Component {
    ...
    selectDigitalBoard = async () => {
        try {
            ...
            let responed = await ServerApi.setConsumerNearDigitalBoard(this.props.navigation.getParam('consumerEmail'), this.state.pressDigitalBoardsId)
            if(this.mountFlag){
        ...
        if (this.state.wating) {
            ...
            return (
                <View style={{ flexDirection: Platform.OS == 'ios' ? 'row' : 'row-reverse', alignItems: 'center', justifyContent: 'space-around', paddingVertical: 20 }}>
                    <View style={{ flex: 3, alignItems: 'center' }}><Text>בחר לוח?</Text></View>
